...
//--------------SIMULOI TEHTÄVÄÄ SIIRTOA------------------------
    ...
    public void teeSiirtoLeikisti(int indeksi, int pelaaja) {
        ...
        int vastustajanMancala;
        int mancala;
        int[] temppi= new int[14];
        
        //kopioidaan tämänhetkinen pelitilanne temp-laudalle
        for(int i=0; i<14; i++){
            temppi[i]=this.lauta[i];
            this.temp.lauta[i]=temppi[i];
        } 
        //Board b = this;
        //this.temp=b;
        
        int kiviaKipossa = temp.lauta[indeksi];

        if (pelaaja == HUMAN) {
            vastustajanMancala = 13;
            mancala = 6;

            this.temp.viimeisenKivenIndeksi(indeksi);
...
//---------LASKEE PELIN LOPPUPISTEET------------------------------------------------------------------------------- 
     ...
     * piste-eroa. Laskee pelitilanteen arvon, vaikkei peli olisi ohi. Silloin oman puolen
...
//------------HAETAAN KAIKKI MAHDOLLISET SIIRROT---------------------------------------------------------------------------
     ...
     * Esim. [1, 1, 0, 0, 1, 0] tarkoittaa, että pelaaja saa siirtää 1., 2. ja 5. kupistaan.
     */
